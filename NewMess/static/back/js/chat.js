...
$(document).on('click', 'li.chat_object', function (e) {
  open_chat(e.currentTarget.id.split('_')[1]);
});

function open_chat(chat_id) {
  var time_ = 'slow';
  const chat_elem = $('#chat_' + chat_id).children();
  var chat = $('.chat-panel');
  if (the_chat_opens) { return 0 };
  if (chat.is(':visible') && chat_id === sessionStorage.getItem('id-open-chat')) {
    chat_elem.removeClass('active-chat');
    sessionStorage.setItem('id-open-chat', 0);
    chat.animate({ width: 'toggle' }, time_);
    $('#from-textarea').val('');
    return 0;
  }
    ...
    chat.animate({ width: 'toggle' }, time_, function () {
      loadChat(chat_id);
    });
  }
  else if (!chat.is(':visible')) {
  ...
  chat_elem.addClass('active-chat');
  sessionStorage.setItem('id-open-chat', chat_id);
  console.log(sessionStorage.getItem('id-open-chat'), chat_id)
  chat.animate({ width: 'toggle' }, time_);
};
...
function set_count_new_message(idChat, value) {
  var count_new_message = $(`#chat_${idChat} .count-new-message-text`);
  if (value == 0) {
    count_new_message.fadeOut('fast', function () {
      $(this).css('background', 'inherit').text('');
    });
  } else {
        ...
        complete: function () {
          $.ajax({
            ...
            complete: function () {
              ...
              setTimeout(function () { the_chat_opens = false; }, 800);
            }
          });
        }
      });
