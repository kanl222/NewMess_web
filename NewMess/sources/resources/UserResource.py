...
class UserResource(Resource):
    ...
    def put(self):
        with db_session.create_session() as session:
            data = request.json
            if 'username' in data and session.query(User).filter(User.username == data['username']).filter(
                    User.id != current_user.id).first():
                abort(Response(f"Username already exists", 400))
            if 'email' in data and session.query(User).filter(User.email == data['email']).filter(
                    User.id != current_user.id).first():
                abort(Response(f"Email already exists", 400))
