...
class ChatParticipantListResource(Resource):
    ...
    def get(self, chat_id=None):
        ...
        with db_session.create_session() as db_sess:
            ...
            users = db_sess.query(User.id, User.username, User.icon).join(ChatParticipant,
                                                                          ChatParticipant.chat_id == chat_id).filter(
                and_(ChatParticipant.user_id == User.id, ChatParticipant.user_id != current_user.id)).all()
